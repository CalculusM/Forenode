...
def render_data_validity_note():
    """데이터 한계와 정직한 메모"""
    with st.expander("⚠️ 데이터 한계"):
        st.markdown("""
        #### 단위
        포장보수현황 `실적` 칼럼은 **단위 미명시**이며, 분포 분석상 **백만원 단위로 추정**
        (평균 60.2 → 6,020만원 · 연간 합계 100~180억원. 도로공사 연간 유지보수 예산 약 2조원 중 포장은 일부).

        #### 신뢰 범위
        - 노선간·사업구분간 **상대 비교**, **운영연차별 패턴**: 신뢰 가능
        - **절대 단가**: 추정치이며, 표준품셈·표준시장단가와 결합 검증 예정
        """)
