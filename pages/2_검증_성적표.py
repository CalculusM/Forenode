...
(파일명은 배선 보존 위해 유지 — '26-07-28 prior 신뢰 근거 재프레임)"""
import os
...
c1.metric("백테스트 채점 완료", f"{len(df)} / 57 사업")
...
c3.metric("Forenode(B) 우위", f"{int((scored['우위']=='B').sum())}건",
          help="교통량 MAE 기준 — 최초 실시협약 예측(A) 대비 보정 효과")
...
# ── ② 대결 차트 ──
st.markdown("##### 📈 보정 전(협약) · 보정 후(Forenode) — 노선별 교통량 MAE (낮을수록 정확)")
...
# ── ④ 미적중 → 보완 매핑 ──
st.markdown("##### 🔧 미적중 구간 → v2 보완 학습 (요약)")
...
st.markdown("##### 🔗 이 기록이 만드는 prior (메인 시뮬레이션의 사전확률)")
st.markdown(
    "- **수요 prior** — 조건부 실현율 분포의 실측 근거: V-009·V-011·V-019 등 (메인 '가정 점검 오버레이'의 밴드가 이 기록에서 나옵니다)\n"
    "- **OPEX 실측 원단위** — 백테스트 7점(경고 2.0~12.0억/km·해상 별도) → OPEX 3모드의 L3 실측 범위\n"
    "- **CAPEX 회귀 참고범위** — 채점 케이스의 계획 대비 실적이 회귀 보정의 근거"
)
try:
    st.page_link("pages/3_학습데이터_출처.py", label="📚 학습 데이터 출처에서 전체 provenance 보기")
except Exception:
    pass
