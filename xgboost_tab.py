...
def render_xgboost_tab():
    ...
    with st.expander("📚 학습 데이터 분석: 등급별 SHAP 패턴"):
        ...
        ※ '26-08-07 데이터 정비 재학습 기준: 파싱 59건 중 학습 47건(개통 전
        건설중 연도 12건은 수익성 지표 부재로 제외), 등급 A 20건·C 27건.
        무형자산상각(관리운영권) 반영으로 EBITDA·DSCR가 재계산되어 종전
        B등급(경계 6건)은 A 또는 C로 재분류되었습니다. 등급 임계 재보정은
        검토 중입니다.

        **A등급 (20건)**
        - 결정 변수: **DSCR_근사** (압도적)
        - 패턴: 총상각 반영 DSCR이 명확히 양극화
        - 예: 신대구부산고속도로
